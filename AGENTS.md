...
**2026-07-10 (cont. 2) — Phosphor atomized, watermark sharpened.** Her notes: watermark unrecognizable at 8px pitch (→ 5px, now clearly a harmonica/whistle), neon glow → satellite-pixel phosphor bloom (shadowBlur now zero app-wide). She predicts neon "is gonna kind of go" — riso is her favorite. Keep that in mind if skins ever need pruning.

**2026-07-10 (cont.) — Digital-lace audit + ghost instrument watermark.** Simone pulled her digital-lace skill for a compliance audit: verdict ~7/12 strict, soul compliant (atoms, paper, real telemetry, dense-sparse), letter violations catalogued (single type voice, >2 signal hues, neon shadowBlur glow, eased motion, alpha-fade dissolves, no threshold edge). Her call: doesn't care about compliance, likes the site as is — wanted one thing, a "super faded" riso instrument deep in the graph background. Shipped: halftone ghost of the SELECTED instrument (harmonica/whistle/voice) behind the grid, noise-ordered edge dissolve, cached per instrument/size/skin. Audit findings parked, on file here if she ever wants the pass (atomized-phosphor neon and the disintegrating trail edge are the two worth doing on their own merits).

**2026-07-10 — Ghost drone exorcised (chain registry + generation guard).** Simone hit "persistent drone though it's turned off": overlapping async play() calls orphaned a chain whose infinite pads no stop() could reach. Fix: all awaits before any node creation, generation counter aborts stale plays, a live-chain registry makes install and stop() exhaustive. Verified by reproducing on old code (RMS residue + stuck state) then measuring 0.000000 RMS post-stop on new. Reminder that bit twice: browser HTTP cache serves stale modules even with the SW cleared — always hard-refresh (cache:'reload' sweep) before verifying.
